...
def ltv():
    ...
    only = orders.merge(order_details)

    receita_media_por_cliente = only.groupby('customer_id').sum()['total'].mean()
    media_pedidos_por_cliente = only.groupby('customer_id').size().mean()
